               ...
               int nDataType, void *pData, int nDataSize, int *pCols, int nCols )
{
    ...
    for ( i = 0; i < nDataSize; i++ )
    {
        ...
        ezd_line( x_hDib, x, y,
                          x + (int)( (double)rad * ezd_cos( pos + v ) ),
                          y + (int)( (double)rad * ezd_sin( pos + v ) ),
                          *pCols );

        ezd_flood_fill( x_hDib, x + (int)( (double)rad / (double)2 * ezd_cos( pos + v / 2 ) ),
                                y + (int)( (double)rad / (double)2 * ezd_sin( pos + v / 2 ) ),
                                *pCols, pCols[ c ] );
